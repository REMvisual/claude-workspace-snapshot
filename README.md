...
- **Project grouping and tab colour are case-sensitive.** Both are derived from the leaf name of the project directory exactly as the agent reported it -- Claude reports the cwd its own process recorded, Codex reports whatever you typed at the prompt. So if the same directory is reached as `c:\standalone\vtwo` in one agent and `C:\Standalone\VTWO` in another, those sessions land in two separate window groups with two different colours. This is deliberate: normalising the colour hash alone was tried and reverted, because it reshuffles the palette for every existing project whose name contains an uppercase letter, and it would only have fixed half the problem anyway -- the group key would still split. Use consistent casing when you `cd` into a project if you want all its tabs in one window.
